...
public class MainActivity extends Activity {
    ...
    // Батч-буфер журнала: при пофайловой ленте (тысячи строк) нельзя постить
    // отдельный Runnable + Html.fromHtml + fullScroll на КАЖДУЮ строку — UI
    // захлебнётся. Копим HTML в буфер и сбрасываем в TextView пачкой раз в ~90мс.
    private final StringBuilder pendingHtml = new StringBuilder();
    ...
    private void append(final String s, final String color) {
        // Готовим строку и складываем в общий HTML-буфер (потокобезопасно).
        String esc = TextUtils.htmlEncode(s).replace(" ", "&#160;");
        synchronized (logLock) {
            logBuf.append(s).append('\n');
            pendingHtml.append("<font color='").append(color).append("'>")
                       .append(esc).append("</font><br>");
            if (flushScheduled) return;      // флаш уже запланирован
            flushScheduled = true;
        }
        // Один отложенный флаш вместо тысяч постов: разом дописываем накопленное
        // в TextView и один раз прокручиваем вниз.
        ui.postDelayed(new Runnable() {
            public void run() {
                String chunk;
                synchronized (logLock) {
                    chunk = pendingHtml.toString();
                    pendingHtml.setLength(0);
                    flushScheduled = false;
                }
                if (chunk.length() > 0) {
                    logView.append(Html.fromHtml(chunk));
    ...
    private void onProgress(final String stage, final int done, final int total, final String file) {
        // ЛЕНТА ФАЙЛОВ: КАЖДЫЙ файл выводим в журнал БЕЗ троттлинга — именно это
        // просит пользователь (видеть каждый smali/xml/png/… при сборке и
        // распаковке). Троттлится только тяжёлое обновление UI-панели ниже.
        ...
        // Троттлинг ТОЛЬКО для обновления полосы/процентов/уведомления, чтобы не
        // перегружать UI-поток десятками кадров в секунду. Стартовый/финальный
        // кадр показываем всегда.
        final boolean edge = (total <= 0) || done <= 0 || done >= total;
        long now = System.currentTimeMillis();
        if (!edge && now - lastProgressUi < 120) return;
        lastProgressUi = now;
