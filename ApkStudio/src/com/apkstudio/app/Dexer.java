...
public class Dexer {
    ...
    public void dex(File classesDir, List<File> libJars, File outDexDir, int minApi) throws Exception {
        ...
        // Панорама файлов, которые уходят в d8 (прогресс распаковки/сборки).
        int pn = 0, ptot = programs.size();
        for (File pf : programs) {
            log.progress("d8: дексирование", pn++, ptot, pf.getName());
        }
        log.progress("d8: дексирование", ptot, ptot, null);
